...
This project implements a multi-turn customer support chatbot built on top of the **Amazon Bedrock AgentCore managed harness**. The chatbot acts as an automated customer support agent for an online shop, handling three distinct customer intent categories through a single, prompt-engineered system prompt:
...
2. **Platform Questions**: Answers common customer questions regarding orders, shipping, returns, and payments using **only** the embedded FAQ document (`online_shop_faq.md` inserted via `{{FAQ}}`). If a question is not covered in the FAQ, it gracefully hands off to human support at `+12345678`.
...
The project infrastructure was deployed in region **`us-east-1`** using AWS CloudFormation and Python setup scripts:
...
## 3. Tool Verification & DynamoDB Record Evidence
...
## 4. Automated Testing & Comparative Evaluation Runs
...
1. **Evaluation Run 1 (`support-chatbot-eval-run-1`)** - *Standard Baseline Suite (`harness_tests_v1.json`)*:
...
2. **Evaluation Run 2 (`support-chatbot-eval-run-2`)** - *Extended Edge Case Suite (`harness_tests_v2.json` / `harness_tests.json`)*:
...
## 5. Submission Screenshots Index

All required evidence screenshots are stored in `screenshots/`:

1. `screenshots/dynamodb_bug_reports.png` — DynamoDB table showing persisted bug report records.
2. `screenshots/bedrock_evaluations_list.png` — Amazon Bedrock Evaluations console showing both completed evaluation runs.
3. `screenshots/bedrock_evaluations_run1.png` — Evaluation Run 1 details (1.00 Correctness score on `harness_tests_v1.json`).
4. `screenshots/bedrock_evaluations_run2.png` — Evaluation Run 2 details (0.75 Correctness score on `harness_tests_v2.json` edge cases).
5. `screenshots/chat_terminal_bug_report.png` — Interactive multi-turn conversation showing step-by-step bug parameter collection and ticket submission.
6. `screenshots/chat_terminal_faq_and_other.png` — Interactive conversation showing covered FAQ answers, uncovered FAQ hand-offs, and out-of-scope request redirects.
